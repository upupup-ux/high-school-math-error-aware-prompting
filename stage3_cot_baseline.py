#!/usr/bin/env python3
"""
Stage 3: Zero-shot CoT Baseline Inference on 400 EAP Test Samples

Supports:
- Qwen2.5-Math-1.5B-Instruct (local)
- Qwen2.5-Math-7B-Instruct (local)
- DeepSeekMath-7B-Instruct (local, with garbled detection)
- DeepSeek-V3 (API via SiliconFlow, with resume from old output)

Usage:
    # Set API key for DeepSeek-V3
    export SILICONFLOW_API_KEY="your_api_key"

    # Run with Qwen 1.5B
    python stage3_cot_baseline.py --model qwen1.5b --test_file /path/to/test.jsonl

    # Run with DeepSeek-V3 (with old output resume)
    python stage3_cot_baseline.py --model deepseek-v3 --test_file /path/to/test.jsonl \\
        --old_output_file /path/to/partial_output.jsonl
"""
...
MODEL_CONFIGS = {
    "qwen1.5b": {
        ...
        "output_suffix": "qwen1.5b_cot_400"
    },
    "qwen7b": {
        "type": "local",
    ...
    "deepseek-math": {
        ...
        "max_new_tokens": 2048,
        "batch_size": 2,
        "use_chat_template": False,          # Use pure text prompt
        "system_prompt": None,
        "output_suffix": "deepseek-math_cot_400",
    ...
    "deepseek-v3": {
        ...
        "output_suffix": "deepseek-v3_cot_400",
        "temperature": 0.0,
        "sleep_interval": 2
    }
}
...
# ===================== Prompt Building =====================
def build_prompt_local(problem, config, tokenizer=None):
...
# ===================== Main =====================
def main():
    parser = argparse.ArgumentParser(description="Stage 3: Zero-shot CoT baseline inference on 400 EAP test samples")
    ...
    parser.add_argument("--old_output_file", type=str, default=None,
                        help="Optional old output file to resume from (for DeepSeek-V3)")
    ...
    # ---- Resume from old output (DeepSeek-V3 only) ----
    if args.old_output_file and os.path.exists(args.old_output_file) and not os.path.exists(output_file):
        print(f"Copying old output file: {args.old_output_file} -> {output_file}")
        import shutil
        shutil.copy2(args.old_output_file, output_file)
    ...
    with open(output_file, "a", encoding="utf-8") as fout:
        for batch_start in range(0, total_rem, config["batch_size"]):
            ...
            if config["type"] == "local":
                prompts = [build_prompt_local(it["problem"], config, tokenizer) for it in batch_items]
